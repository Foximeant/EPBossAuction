...
-- ============================================================
-- epgp.lua — мост к внешнему EPGP-аддону
-- ============================================================
-- EPBossAuction сам не считает EP/GP — эти цифры берутся из
-- ДРУГОГО установленного у игрока аддона (EPGP, EPGP_Auction,
-- CEPGP или EPGPCore — под разными именами по факту один и тот же
-- класс аддонов). Т.к. у них может отличаться API, код пробует
-- по очереди несколько способов получить EP игрока:
--   1) epgpTable:GetEPGP(playerName) — основной, современный способ
--   2) epgpTable.db.profile.players[playerName].EP — прямой доступ
--      к SavedVariables EPGP, если метода GetEPGP нет
--   3) epgpTable:GetEP(playerName) — старый/альтернативный метод
-- Если ни один не сработал — EP считается неизвестным, игроку
-- показывается предупреждение в чат (см. DoUpdateMyEP/ForceEPUpdate).
--
-- Твинки: если EPGP-аддон возвращает поле "main" (это твинк такого-то
-- мейна), EP берётся у мейна, а не у самого твинка — так у альтов
-- показывается тот же банк EP, что и у основного персонажа.
--
-- FindEPGP() — отладочная функция (сканирует _G в поисках подходящих
